...
void dp_init_com_vars(void)
{
    ...
    end_page_address = 0u;
    // Jan Sucan
    // Pred operaciou je potrebne inicializovat aj dalsie premenne suvisiace so strankovanim.
    // Ak sa neinicializuje image_size, moze sa pri aktualnej operacii vyziadat viac dat z .DAT suboru
    // (hodnota pouzita predchadzajucou operaciou) nez ako ma .DAT subor pre aktualnu operaciu.
...
void dp_get_page_data(DPULONG image_requested_address)
{
    ...
    */
	dcw_get_page_data_from_external_storage(image_requested_address, &return_bytes, &start_page_address, &end_page_address);
...
DPUCHAR * dp_get_data_block_element_address(DPULONG bit_index)
{
    ...
    /* Jan Sucan: removed && ((image_requested_address + MIN_VALID_BYTES_IN_PAGE) <= end_page_address) */
    if (
